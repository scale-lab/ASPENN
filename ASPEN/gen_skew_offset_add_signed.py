...
def gen_skew_offset_add_signed(size_input,size_code):
	verilog_code = []
...
module skew_offset_add_signed
	...
	begin
		...
		// 2s Complement of most significant offset bits
		signed_offset_msb <= ~offset[(size_input-1)*size_code +: size_code] + 1;
		for (j = 0; j < size_code; j = j + 1)
		begin
			skewed_offset[j*size_skew + (size_input-1) + j] <= signed_offset_msb[j];
		end
	...
	if (num_block % 2) == 0:
		size_add0 = size_partition*num_partition+size_block-1
		size_add1 = size_partition*num_partition+size_block-1
	else:
		size_add0 = size_partition*num_partition-1
		size_add1 = size_partition*num_partition-1
	...
	if (num_block % 2) == 0:
		...
		begin
			add0[size_partition*num_partition+i] = signed_offset_msb[size_code-1] || offset[size_input*size_code-1];
		end""")
	else:
		verilog_code.append(
		f"""\
		for (i = 0; i < size_block; i = i + 1)
		begin
			add1[size_partition*(num_partition-1)+size_block+i] = signed_offset_msb[size_code-1] || offset[size_input*size_code-1];
		end""")
	...
	with open("skew_offset_add_signed.v", "w") as outfile:
		outfile.write("\n".join(verilog_code))


def build_arg_parser():
	parser = argparse.ArgumentParser(description='Generates Verilog code for a signed skewed offset adder ')
...
if __name__ == '__main__':
	...
	gen_skew_offset_add_signed(size_input,size_code)
